app.host = {
  ...
  displayQR: function() {
    let queryUrl =
      "http://api.qrserver.com/v1/create-qr-code/?data=" + this.guestUrl;
  ...
  getPlaylistInfo: function() {
    ...
    $.ajax({
      data: {
        fields:
          "images,name,uri,tracks.items(track(id,name,album(images),artists))"
      },
      method: "GET",
      url:
        "https://api.spotify.com/v1/playlists/" +
        self.configuration.spotifyPlaylistId
    })
      .done(function(response) {
        self.playlistUri = response.uri;
        self.displayPlaylistInfo(response);
        self.listenForTrackCount();
  ...
  listenForNewTracks: function() {
    ...
    app.playlistRef
      ...
      .on("child_added", function(trackSnapshot) {
  ...
  appendTrack: function({ artist, image, id, name }) {
    ...
    let track = $("<div>")
      ...
      .append(
        $("<img>")
          .attr("src", image)
          .addClass("align-self-center mr-3")
      )
      .append(trackBody);
